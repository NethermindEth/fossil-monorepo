...
impl Default for ProofVerifierConfig {
    fn default() -> Self {
        Self {
            ...
            verifier_contract_address: std::env::var("PITCHLAKE_VERIFIER_CONTRACT").unwrap_or_else(
                |_| {
                    warn!("PITCHLAKE_VERIFIER_CONTRACT not set, using default");
                    "0x0".to_string()
                },
            ),
            verify_onchain: std::env::var("VERIFY_PROOFS_ONCHAIN")
...
impl<G: Risc0ProofGenerator + Send + Sync> ProofVerifier for IntegratedProofVerifier<G> {
    ...
    async fn generate_and_verify_proof(
        ...
        let onchain_tx_hash = if config.verify_onchain {
            ...
            if config.verifier_contract_address == "0x0" {
                warn!("Verifier contract address is 0x0, skipping onchain verification");
                None
            } else {
                match self
...
mod tests {
    ...
    fn test_proof_verifier_config_default() {
        ...
        assert_eq!(config.verifier_contract_address, "0x0");
    }
